...
func DefaultFits(gft *fst.GoFast) *fst.GoFast {
	gft.Fit(mid.FitMaxConnections(gft.FitMaxConnections))     // 最大同时接收请求数量
	gft.Fit(mid.FitMaxContentLength(gft.FitMaxContentLength)) // 最大的请求头限制，默认32MB
...
func DefaultHandlers(gft *fst.GoFast) *fst.GoFast {
	...
	gft.Before(mid.MaxContentLength)               // 最大的请求头限制，默认32MB（这个可以单独限制不同的路径）
	gft.Before(mid.Gunzip)                         // 自动 gunzip 解压缩（前面的处理都完成了再解压缩）
