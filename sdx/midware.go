...
func SuperHandlers(app *fst.GoFast) *fst.GoFast {
	...
	otherPaths := app.SpecialRoutePaths()
	spcLen := uint16(len(otherPaths))
	otherPaths = append(otherPaths, "Count0")
	otherPaths = append(otherPaths, "Count1")
	otherPaths = append(otherPaths, "Count2")
	...
	app.OnBeforeBuildRoutes(func(app *fst.GoFast) {
		...
		reqKeeper.StartWorking(app.RoutePathsWithMethod(), otherPaths) // 警卫上岗
	})
	...
	app.UseHttpHandler(mid.HttpReqCountPos(reqKeeper, spcLen+0))       // 访问计数1
	...
	app.UseHttpHandler(mid.HttpReqCountPos(reqKeeper, spcLen+1))       // 访问计数2

	// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	// 第二级：ContextHandlers 带上下文 fst.Context 的执行链
	app.Before(mid.ReqCountPos(reqKeeper, spcLen+2)) // 正确匹配路由的请求数
	...
	if cnf.UseSpecialHandlers {
		app.SpecialBefore(mid.ReqCount(reqKeeper)) // 数量统计
