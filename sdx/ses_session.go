...
func (ss *Session) Init() {
	...
	//}
	if ss.Redis == nil {
		ss.Redis = gfrds.NewGoRedis(&ss.ConnCnf)
	}
...
func SdxSessBuilder(ctx *fst.Context) {
	...
	ctx.Sess.Values = make(map[string]any)
	tok := ctx.Pms["tok"]
	if tok == nil {
		tok = ""
	}
	...
	isValid := MySS.checkToken(reqSid, reqHmac, ctx)

	// 按照ip计算出当前hmac，和请求中的hmac相比较，看是否相等
	// 如果Sid验证通过
	if isValid || MySS.CheckTokenIP == false {
		ctx.Sess.Sid = reqSid
	}
	...
	if ctx.Sess.Sid == "" {
		...
		MySS.loadSessionFromRedis(ctx) // 通过sid 到 redis 中获取当前 session
	}
}

// 验证是否登录
func SdxMustLogin(ctx *fst.Context) {
	uid := ctx.Sess.Get(MySS.AuthField)
	if uid == nil || uid == "" {
...
func (ss *Session) newToken(ctx *fst.Context) (string, string) {
	return genToken(ss.Secret + ctx.ClientIP())
}

// 利用当前 sid 和 ctx 中包含的 request_ip | 计算出hmac值，然后和token中携带的 hmac值比较，来得出合法性
func (ss *Session) checkToken(sid, sHmac string, ctx *fst.Context) bool {
