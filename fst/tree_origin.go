...
func (n *radixNode) regSegment(mTree *methodTree, nParent *radixNode, seg string, ri *RouteItem) {
	...
	// 新来的只可能绑定在 n 节点的父节点上
	if i == 0 {
		cst.PanicIf(nParent == nil, "解析路由错误，找不到父节点")
...
func (n *radixNode) bindSegment(mTree *methodTree, path string, ri *RouteItem) {
	...
	cst.PanicIf(wildCt != slashCt, "路由配置有误，通配符标识符只能成段出现")
	...
	for i := 0; i < lenParts; i++ {
		tSeg = wildParts[i]
		cst.PanicIf(tSeg[0] == '*' && i < (lenParts-1), "通配符*只能出现在最后一段")

		if i == 0 {
			if isBeginWildcard(tSeg) {
				cst.PanicIf(len(tSeg) < 2, "通配符必须设置参数名称")
				pNode.nType = param
				if tSeg[0] == '*' {
